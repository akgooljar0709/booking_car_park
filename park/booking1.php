            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="different-sign-in">
                        ...
                        <h4>Book A Park !</h4>
                        <form action="make_booking.php" id="parking-reservation-form" method="post">
                          <label for="spot-select">Select a parking spot:</label>
                          <select id="parking-spot-select" name="spot_id">
...
const form = $('#parking-reservation-form');
const startTimeSelect = $('#start-time-select');
const endTimeSelect = $('#end-time-select');

// Send an AJAX request to get the available times
$.get('get_available_times.php', { spotId: $('#parking-spot-select').val() }, function(data) {
    // Clear the current options in the select elements
    startTimeSelect.empty();
    endTimeSelect.empty();

    // Add the available times as options to the select elements
    $.each(data.start_times, function(i, time) {
        startTimeSelect.append($('<option>', { value: time, text: time }));
    });
    $.each(data.end_times, function(i, time) {
        endTimeSelect.append($('<option>', { value: time, text: time }));
    });
});
